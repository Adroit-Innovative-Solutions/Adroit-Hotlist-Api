...
public interface PlacementRepository extends JpaRepository<PlacementDetails, String> {
    ...
    boolean existsByConsultantEmailId(String consultantEmailId);
    boolean existsByConsultantContactNo(String consultantContactNo);
    ...
    @Query("SELECT p FROM PlacementDetails p WHERE (p.createdAt BETWEEN :startDate AND :endDate) OR (p.startDate BETWEEN :startDate AND :endDate)")
    ...
    // fixed: consultantContactNo instead of candidateContactNo
    PlacementDetails findByConsultantContactNoAndClientName(String consultantContactNo, String clientName);
    ...
    // fixed: consultantEmailId instead of candidateEmailId
    List<PlacementDetails> findByConsultantEmailId(String consultantEmailId);

    List<PlacementDetails> findByConsultantEmailIdAndCreatedAtBetween(String consultantEmailId,
                                                                      LocalDate startDate,
                                                                      LocalDate endDate);

    // For non-email/fetch with login status
    List<PlacementDetails> findPlacementsByStartDateBetweenOrStartDateAfter(LocalDate start,
                                                                            LocalDate end,
                                                                            LocalDate afterDate);

    // fixed: consultantEmailId instead of candidateEmailId
    List<PlacementDetails> findByConsultantEmailIdAndStartDateBetweenOrStartDateAfter(String consultantEmailId,
                                                                                      LocalDate start,
                                                                                      LocalDate end,
                                                                                      LocalDate afterDate);
}
